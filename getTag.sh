...
if [ $CNTBIB -ne "0" ]
   then
	...
	$BIN/addSubfield.pl 590 ${BIBMARC}.addsubf.${cntr} ${BIBMARC}.${cntr} >> $LOG 2>&1
	$BIN/addTag.pl 500 590 ${BIBMARC}.addsubf_addt.${cntr}  ${BIBMARC}.addsubf.${cntr}  >> $LOG 2>&1


	$BIN/taghold.pl --ifmt=marc --ofmt=text ${BIBMARC}.addsubf_addt.${cntr} >  ${BIBMARC}.${cntr}.cp 2>> $LOG
	$BIN/taghold.pl --ifmt=text --ofmt=marc ${BIBMARC}.${cntr}.cp >  ${BIBMARC}.${cntr}.impt 2>> $LOG 
